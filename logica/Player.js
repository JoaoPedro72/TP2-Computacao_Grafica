import { AviaoModelo } from "../modelos/aviao.js";
import { Utills } from "../Utills.js";
const utills = new Utills();

export class Player {
    constructor(pos = [], keys = {}, model = new AviaoModelo){
        this.pos = pos;
        this.keys = keys;
        this.angulo = 0;
        this.anguloFaixa = 0;
        this.velocidade = 4;
        this.velCurva = (10 - this.velocidade)*2;
        this.model = model;
    }
    tick(time){
        this.move(time);
        this.faixaVirar(time);
    }
    move(time){
        this.pos[0] -= this.velocidade * Math.sin(utills.radians(this.angulo)) * time;
        this.pos[2] -= this.velocidade * Math.cos(utills.radians(this.angulo)) * time;

        if(this.keys.w && this.velocidade < 10){
            this.velocidade += time;
            this.velCurva = (10 - this.velocidade)*2
        }
        if(this.keys.s && this.velocidade > 1){
            this.velocidade -= time;
            this.velCurva = (10 - this.velocidade*0.5)*2
        }
        console.log(this.velocidade);
        if(this.keys.a) {
            this.angulo += this.velCurva * time;
            this.anguloFaixa -= 0.1 * time * this.velocidade/2;
        }
        if(this.keys.d) {
            this.angulo -= this.velCurva * time;
            this.anguloFaixa += 0.1 * time * this.velocidade/2;
        }
    }
    faixaVirar(time){
        this.model.root.rot[2] = -this.anguloFaixa;
        this.model.faixa1.rot[2] = this.anguloFaixa;
        this.model.faixa1.rot[1] = Math.sin(time)/8 + this.anguloFaixa;
        this.model.faixa2.rot[1] = -Math.sin(time)/4 + this.anguloFaixa;
        this.model.faixa3.rot[1] = Math.sin(time)/4 + this.anguloFaixa;

        console.log(this.anguloFaixa);

        if(this.anguloFaixa > 0) {
            this.anguloFaixa -= time * this.anguloFaixa * this.velocidade/4;
            if(this.anguloFaixa < 0) this.anguloFaixa = 0;
        }
        else if(this.anguloFaixa < 0) {
            this.anguloFaixa -= time * this.anguloFaixa * this.velocidade/4;
            if(this.anguloFaixa > 0) this.anguloFaixa = 0;
        }

        //console.log(this.anguloFaixa);
    }
}